...
class OptecTCFS (FocuserBase):
    ...
    def _move(self, direction, steps):
        ...
        self.log.debug('Moving focuser %i steps on %s direction' % (steps, repr(self._directions[direction])))
    ...
    def _setMode(self, mode):
        ...
        if not ack:
            raise IOError("Error trying to set focuser mode to '%s' (read timeout)." % self._modes[mode])

        if not ack.startswith('!'):
            raise IOError(
                "Error setting focuser mode to '%s' (bad ack %s)." % (mode, repr(ack)))
    ...
    def _readline(self):
        ...
        if ret.startswith('ER=1'):
            self.log.error("Focuser temperature probe seems not properly attached")
            ret = ret.split("\n")[-2]
